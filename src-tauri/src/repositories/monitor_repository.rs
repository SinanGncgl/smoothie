...
impl<'a> MonitorRepository<'a> {
  ...
  /// Create a new monitor with full metadata
  ...
  /// Create a new monitor with extended metadata
  #[allow(clippy::too_many_arguments)]
  ...
  pub async fn update_position(
    ...
    sqlx::query(
      "UPDATE monitors SET x = $1, y = $2, width = $3, height = $4, updated_at = $5 WHERE id = $6",
    )
    .bind(x)
    .bind(y)
    .bind(width)
  ...
  pub async fn count_by_profile_id(&self, profile_id: Uuid) -> Result<i64> {
    let (count,) =
      sqlx::query_as::<_, (i64,)>("SELECT COUNT(*) FROM monitors WHERE profile_id = $1")
        .bind(profile_id)
        .fetch_one(self.pool)
        .await
        .map_err(|e| SmoothieError::DatabaseError(e.to_string()))?;
