...
pub async fn create_rule(
    ...
    let rule = AutomationService::create_rule(&state.db, &profile_id, rule_type, trigger_config).await?;
...
pub async fn get_rules(
    ...
    let data: Vec<serde_json::Value> = rules.into_iter().map(|r| serde_json::to_value(r).unwrap()).collect();
