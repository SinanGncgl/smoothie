...
pub async fn get_browser_tabs(
    ...
    let data: Vec<serde_json::Value> = tabs.into_iter().map(|t| serde_json::to_value(t).unwrap()).collect();
...
pub async fn open_tabs(
    ...
    tracing::info!("Opening {} browser tabs for profile {}", data.len(), profile_id);
